...
function SignUp() {
    ...
    const onSubmit = async (data) => {
        ...
        const cpwd = data.password2;
        try {
            const response = await axios.post(SIGNUP_URL,JSON.stringify({firstName,lastName,user,pwd,cpwd}),{
            ...
            <form onSubmit={handleSubmit(onSubmit)} noValidate className="w-full max-w-lg">
                ...
                <div className="flex flex-col items-center space-y-4 w-full">
                    <p className="text-sm font-light text-gray-500 dark:text-gray-400 text-center">
                        Already have an account? <Link className="font-medium text-blue-600 hover:underline dark:text-blue-500" to="/login">Log in</Link>
                    </p>
                    <button className="landingButton1">
                        Sign up
                    </button>
                </div>
            </form>
